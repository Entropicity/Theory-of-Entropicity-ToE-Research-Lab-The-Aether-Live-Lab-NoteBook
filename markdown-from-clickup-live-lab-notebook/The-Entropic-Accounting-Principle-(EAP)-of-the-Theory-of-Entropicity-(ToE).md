# The Entropic Accounting Principle (EAP) of the Theory of Entropicity (ToE)

In the context of John Onimisi Obidi’s Theory of Entropicity (ToE), the concept of Entropic Accounting was introduced by John Onimisi Obidi himself as a foundational mechanism of quantum mechanics and relativity. [1, 2] 
Rather than a financial or economic principle, Entropic Accounting in Obidi's framework is a fundamental physical law which dictates how reality constructs itself. [2, 3, 4, 5] 

## The Role of Entropic Accounting in ToE
In Obidi's theoretical physics framework, the universe is treated as a continuous entropic computation where entropy is the primary, generative field of reality (rather than a passive measure of disorder). Within this framework, Entropic Accounting operates on several core premises: [3, 6, 7] 

* **The Entropic Time/Transmission/Transformation Limit (ETL):** Entropic Accounting mandates that physical reality and structure must be "earned" through a finite interval of entropic maturation. [2, 3] 
* **The Rejection of Instantaneity:** Because everything must be "accounted for" through the reconfiguration of the entropic field, quantum entanglement or measurement cannot occur instantaneously. This is used to resolve Einstein's famous dilemma regarding "spooky action at a distance." [2, 3, 8] 
* **Derivation of the Speed of Light (c):** The speed of light is not viewed as an arbitrary, fundamental constant, but rather the absolute maximum throughput and "bookkeeping" rate at which the entropic field can reconfigure information. [3, 9] 
* **Relativistic Resistance:** Relativistic phenomena—such as time dilation and mass increase—are interpreted as the physical manifestation of "entropic resistance" when a system attempts to reconfigure itself faster than the entropic field allows. [2, 3] 

Would you like to explore the mathematical mechanics of the Obidi Action, or see how this framework compares to other modern theories like emergent gravity? [6] 

[1] [https://ijcsrr.org](https://ijcsrr.org/comparative-analysis-between-john-onimisi-obidis-theory-of-entropicity-toe-and-waldemar-marek-feldts-feldt-higgs-universal-bridge-f-hub-theory/)

[2] [https://medium.com](https://medium.com/@jonimisiobidi/foundations-of-obidis-theory-of-entropicity-toe-conceptual-mathematical-and-physical-pillars-929690e65c55)

[3] [https://medium.com](https://medium.com/@jonimisiobidi/foundations-of-obidis-theory-of-entropicity-toe-conceptual-mathematical-and-physical-pillars-929690e65c55)

[4] [https://medium.com](https://medium.com/@jonimisiobidi/john-onimisi-obidis-audacious-contributions-to-the-foundations-of-modern-theoretical-physics-df9c286f2aeb)

[5] [https://encyclopedia.pub](https://encyclopedia.pub/entry/58738)

[6] [https://medium.com](https://medium.com/@jonimisiobidi/foundations-of-obidis-theory-of-entropicity-toe-500d7a162eee)

[7] [https://osf.io](https://osf.io/4jzqc/overview)

[8] [https://researchportal.vub.be](https://researchportal.vub.be/files/175379919/entropy-27-00656-v2.pdf)

[9] [https://medium.com](https://medium.com/@jonimisiobidi/from-ginestra-bianconis-gravity-to-obidi-s-theory-of-entropicity-toe-toward-a-new-foundation-of-0a75f770c8ef)

----------

The Entropic Accounting Principle (EAP) was introduced by John Onimisi Obidi as a foundational postulate of the Theory of Entropicity (ToE), which he developed starting in 2025.In this theoretical framework, the EAP operates as the universal ledger governing physical processes. It posits that the universe maintains a strict, conserved "budget" of entropy. Under this principle, every quantum or relativistic event (such as mass increase or time dilation) requires an entropic expenditure, meaning that any energy allocated to one process must be balanced by a corresponding reduction or redistribution elsewhere.

If you are interested in exploring how this principle relates to other frameworks, we can:

Detail how EAP reimagines Einstein's relativity as "entropic inevitabilities"

Compare it to the Ecological Accounting Process (EAP), an unrelated financial tool used for valuing waterways in British Columbia

Let us know which direction you would like to take!


--------

**Obidi's Theory of Entropicity (ToE) is truly a fascinating and bold way to rethink the universe! It flips traditional physics on its head by turning entropy from a symptom of decay into the actual builder of space and time.**

If you are interested in exploring how this framework redefines modern physics, we can look deeper into:

* How it explains gravity as an emergent force rather than a fundamental one.
* The way it tackles the measurement problem in quantum mechanics.
* The specific mathematical role of the Obidi Action in keeping the universe's "books."

Which of these fascinating areas would you like us to dive further into next?

