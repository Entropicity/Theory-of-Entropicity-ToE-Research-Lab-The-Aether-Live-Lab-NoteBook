...
A disformal scaling factor (traditionally scaled exponentially as $e^{S/k_B}$). This transforms the purely statistical information metrics into a physical metric-affine geometry. [2, 9] 

### The $\alpha = 0$ Connection: 
The theory utilizes the Amari-Čencov $\alpha$-connections from information geometry. By isolating the $\alpha = 0$ state, it naturally unifies with the torsion-free Levi-Civita connection required by standard General Relativity macro-spacetime. [6, 13] 
...
Expressed through the equation $\hbar c = k_B T_S \ell_S$. This bridge establishes a direct mathematical link between quantum mechanics ($\hbar$), relativity ($c$), thermodynamics ($k_B$), and information geometry ($\ell_S$). [1, 7, 14, 15] 
